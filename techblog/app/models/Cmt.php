...
class Cmt
{
    ...
    {
        $this->db->query('INSERT INTO cmts (cmt_body, user_id, post_id) VALUES (:cmt_body, :user_id, :post_id)');
    ...
    {
        $this->db->query('SELECT cmt_id, cmt_body, user_name, user_photo, cmt_created FROM cmts, users WHERE cmts.user_id = users.user_id and post_id = :id ORDER BY cmt_created DESC');
        $this->db->bind(':id', $id);
        return  $this->db->resultSet();
    }
    ...
    {
        $this->db->query('SELECT cmt_id, cmt_body, user_name, user_photo, cmt_created FROM cmts, users WHERE cmts.user_id = users.user_id and user_id = :id');
        $this->db->bind(':id', $id);
        return $this->db->resultSet();
    }
}